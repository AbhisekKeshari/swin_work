...
def train_epoch(config, epoch, model, save_output, criterion, optimizer, scheduler,
                train_loader):
    losses = []
    model.train()
    ...
    for data in tqdm(train_loader):
        ...
        pred = model(r_img, d_img,save_output)
    ...
    return np.mean(losses), rho_s, rho_p, model.state_dict(), optimizer.state_dict(),scheduler.state_dict()


""" validation """


def eval_epoch(config, epoch, model, save_output, criterion, test_loader, scheduler, train_loader):
    with torch.no_grad():
        losses = []
        model.eval()
